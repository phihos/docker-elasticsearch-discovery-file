...
run_as_other_user_if_needed() {
    if [[ "$(id -u)" == "0" ]]; then
        ...
        exec chroot --userspec=elasticsearch / "${@}"
    else
...
if [[ "$1" != "eswrapper" ]]; then
    if [[ "$(id -u)" == "0" && $(basename "$1") == "elasticsearch" ]]; then
        ...
        set -- "elasticsearch" "${@:2}"
        # Use chroot to switch to UID elasticsearch
        exec chroot --userspec=elasticsearch / "$@"
    else
...
run_as_other_user_if_needed /usr/share/elasticsearch/bin/elasticsearch -Epath.conf=/etc/elasticsearch "${es_opts[@]}"